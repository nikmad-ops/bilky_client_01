...
async function main() {
  ...
  log(`READ ONLY TARGET_DATE=${targetDate}`);
  log("READ ONLY MODE. NO FICHAR/SIGN ACTIONS WILL BE CALLED.");
  ...
  try {
    ...
    const cardText = await page
      .locator(`#container_${targetDate}`)
      .innerText();

    log("----- RAW CARD TEXT -----");
    console.log(cardText);
    log("----- END RAW CARD TEXT -----");

    const message = [
      `🔎 Bilky READ ONLY — ${CLIENT_NAME} ${displayDate(targetDate)}`,
      "",
    ...
    log("READ ONLY SUCCESS");
  } catch (error) {
    console.error(
      `READ ONLY FAILED: ${error.message}`
    );

    try {
      await page.screenshot({
        path: "diagnostics/read-only-error.png",
        fullPage: true,
      });
    } catch {}
    ...
    try {
      await sendTelegramTo(
        ADMIN_TELEGRAM_BOT_TOKEN,
        ADMIN_TELEGRAM_CHAT_ID,
        `❌ Bilky READ ONLY — ${CLIENT_NAME} ${displayDate(targetDate)}. ERROR: ${error.message}. No Fichar/Sign action was performed.`,
        "Admin"
      );
    } catch {}
